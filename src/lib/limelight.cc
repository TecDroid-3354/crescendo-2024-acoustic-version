...
init() noexcept -> void {
    ...
    // cs::HttpCamera camera { "Limelight Stream", "http://frcvision.local:1181/stream.mjpg" };
    // frc::CameraServer::AddCamera(camera);

    frc::ShuffleboardTab &lltab = frc::Shuffleboard::GetTab(k::str::limelight_entity_tab);
    // lltab.Add(camera);
